...
contract EtherFunFactory is ReentrancyGuard {
    ...
    // Ethervista pool launch contract address
    ...
    function createSale(
        ...
        if (msg.value > 0) {
            ...
            // 確定有要買的同時才會create sale contract 因為要把gas fee轉嫁給第一個使用者
            assembly {
            ...
            // 記錄第一個買的人 最後會回傳 0.02 ether 給他
            firstBuyer[tokenAddress] = msg.sender;

            uint256 minTokensOut = 0;
            // 建立後call buy function
            (uint256 totalRaised, uint256 tokenBalance) = ISaleContract(
                tokenAddress
            ).buy{value: msg.value}(msg.sender, minTokensOut);
    ...
    function buyToken(
        ...
        if (firstBuyer[tokenAddress] == address(0)) {
            bytes32 salt = keccak256(
                abi.encodePacked(sale.creator, sale.creationNonce)
            );
    ...
    function sellToken(
        ...
        (uint256 totalRaised, uint256 tokenBalance) = ISaleContract(
            tokenAddress
        ).sell(msg.sender, tokenAmount, minEthOut);
        sale.totalRaised = totalRaised;
    ...
    /// @notice Predict the address of the token contract that will be created
    ...
    function updateParameters(
        ...
        require(
            _launchContractAddress != address(0),
            "Invalid launch contract"
        );
